...
PhysicsSystem::PhysicsSystem(GameWorld& g) : gameWorld(g)	{
	...
	SetGravity(Vector3(0.0f, -98, 0.0f));
}
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float cRestitution = (a.GetPhysicsObject()->getElasticity() + b.GetPhysicsObject()->getElasticity()) /2;
	...
	float cFriction = (a.GetPhysicsObject()->getFriction() + b.GetPhysicsObject()->getFriction()) /2;
	...
	float frictionEffect = Vector3::Dot(frictionInertiaA + frictionInertiaB, t);
	float jt = -(cFriction * frictionForce) / (totalMass + frictionEffect);
